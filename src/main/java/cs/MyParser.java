 ...
 */
public class MyParser extends AbstractRDFParser {
     ...
     */
    public MyParser(ValueFactory valueFactory) {
        super(valueFactory);
    }
        ...
        try {
            ...
            while (c != -1) {
                if (c == '#') {
                    ...
                    int position = parseTriple(c);
                    if (position == -999) {
                        //System.out.println("SKIPPED LINE: " + lineNo);
                        invalidLineNumbers.add(lineNo);
                        c = skipLine(c);
                    } else {
                        c = position;
                    }
    ...
    private int parseTriple(int c) throws IOException, RDFParseException, RDFHandlerException {
        ...
        try {
            int parseSubjectPointer = parseSubject(c);
            if (parseSubjectPointer == -999) {
                return -999;
            } else {
                c = parseSubjectPointer;
            }
            
            c = skipWhitespace(c);
            
            int parsePredicatePointer = parsePredicate(c);
            if (parsePredicatePointer == -999) {
                return -999;
            } else {
                c = parsePredicatePointer;
            }
            
            c = skipWhitespace(c);
            
            int parseObjectPointer = parseObject(c);
            if (parseObjectPointer == -999) {
                return -999;
            } else {
                c = parseObjectPointer;
            }
